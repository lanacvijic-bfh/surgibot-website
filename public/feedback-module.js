...
function getCurrentVignette() {
  const stored = localStorage.getItem(STORAGE_KEYS.vignette);
  if (!stored) return null;
  try {
    return JSON.parse(stored);
  } catch (e) {
    console.error("Failed to parse vignette:", e);
    return null;
  }
}
...
function computeCompleteness(coverageChecklist) {
  // covered = 1, partially = 0.5, not_covered = 0
                ...
                ${safetyFlags.map((f) => `
                  <div class="bg-red-50 border border-red-200 rounded-xl p-4">
                    <div class="flex items-start justify-between gap-3">
                      ...
                      <span class="px-3 py-1 rounded-full text-xs font-semibold border ${getRatingBadge(
                        String(f.severity || "low").toLowerCase() === "high"
                          ? "Poor"
                          : String(f.severity || "low").toLowerCase() === "medium"
                            ? "Needs Improvement"
                            : "Satisfactory"
                      )}">
                        ${escapeHtml(String(f.severity || "low").toUpperCase())}
                      </span>
                    </div>
